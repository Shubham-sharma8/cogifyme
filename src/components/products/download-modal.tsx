...
export const DownloadModal = ({ isOpen, onClose }: DownloadModalProps) => {
  ...
  const handleDownload = () => {
    ...
    if (siteConfig.emdoc.downloadUrl) {
      ...
      // Fallback: trigger a client-side generated release info packet or binary stub
...
`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-xl bg-zinc-900 border border-white/10 rounded-2xl shadow-2xl p-6 sm:p-8 overflow-hidden text-zinc-100">
        {/* Background glow */}
        ...
        <div className="flex items-center space-x-4 mb-6">
          <div className="relative w-16 h-16 rounded-2xl overflow-hidden border border-white/15 shadow-lg bg-zinc-800 shrink-0">
            <Image
        ...
        {/* Privacy Callout */}
        <div className="p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 mb-6 flex items-start gap-3">
          <ShieldCheck className="w-5 h-5 text-emerald-400 shrink-0 mt-0.5" />
          <div className="text-xs text-emerald-200/90 leading-relaxed">
            <strong className="text-emerald-300 block mb-0.5">100% Air-Gapped Zero-Cloud Guarantee</strong>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6 text-xs">
          <div className="p-3 rounded-lg bg-zinc-950/60 border border-white/5">
            ...
            <span className="font-medium text-zinc-200">macOS 13.0+</span>
            <span className="text-zinc-400 block text-[11px] mt-0.5">Ventura, Sonoma, Sequoia</span>
          </div>
          <div className="p-3 rounded-lg bg-zinc-950/60 border border-white/5">
            <span className="text-zinc-500 block mb-1">Architecture</span>
            <span className="font-medium text-zinc-200">Universal Binary</span>
            <span className="text-zinc-400 block text-[11px] mt-0.5">Apple Silicon & Intel x86_64</span>
          </div>
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              >
                <div className="text-xs font-bold text-zinc-200">{option.label}</div>
        ...
        <div className="space-y-3">
          <button
            ...
            className="w-full py-3.5 px-6 rounded-xl bg-gradient-to-r from-indigo-500 via-indigo-600 to-blue-600 hover:from-indigo-400 hover:to-blue-500 text-white font-semibold text-sm transition-all duration-200 shadow-lg shadow-indigo-600/25 flex items-center justify-center gap-2 cursor-pointer"
          >
            <Download className="w-4 h-4" />
            <span>Download EmDoc for macOS</span>
            <span className="text-xs font-normal opacity-75 font-mono">
              ({arch === "universal" ? "Universal DMG" : arch.toUpperCase()})
            </span>
          </button>

          {downloadStarted && (
            <div className="p-3 rounded-lg bg-zinc-950/80 border border-indigo-500/30 text-xs text-indigo-300 flex items-center gap-2 animate-in fade-in">
          ...
          <div className="flex items-center justify-between text-[11px] text-zinc-500 pt-2 border-t border-white/5">
            <span className="flex items-center gap-1">
              ...
              Configurable via <code className="text-zinc-400">EMDOC_DOWNLOAD_URL</code>
            </span>
